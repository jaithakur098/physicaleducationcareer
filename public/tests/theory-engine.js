...
async function initEngine() {
  ...
  try {
    ...
    if (localData) {
      ...
      // Try multiple candidate paths so the engine works regardless of how the
      // test page is served (relative, absolute, or config-driven).
      const cfgPath = (currentChapterConfig && currentChapterConfig.path) || "";
      const candidates = [
        `../data/${klass}/${subject}/${chapter}.js`,
        `/data/${klass}/${subject}/${chapter}.js`,
        `data/${klass}/${subject}/${chapter}.js`,
        cfgPath
      ].filter(Boolean);
      ...
      for (const url of candidates) {
        try {
          ...
          if (!response.ok) { lastErr = new Error(`HTTP ${response.status} for ${url}`); continue; }
          let text = await response.text();
          // Support const/let/var questionBank OR questions (legacy files).
          text = text.replace(/(?:^|\n)\s*(?:export\s+)?(?:const|let|var)\s+questionBank\s*=/, "\nwindow.questionBank =");
          text = text.replace(/(?:^|\n)\s*(?:export\s+)?(?:const|let|var)\s+questions\s*=/, "\nwindow.questionBank =");
          const scriptEl = document.createElement("script");
          scriptEl.text = text;
          document.head.appendChild(scriptEl);
          if (Array.isArray(window.questionBank)) { loaded = true; console.log(`Loaded ${window.questionBank.length} questions from ${url}`); break; }
        } catch (e) { lastErr = e; }
      }
      if (!loaded) {
        console.warn("No question bank could be loaded for", klass, subject, chapter, lastErr);
        console.warn("Using standard theory fallback pool.");
        window.questionBank = [
  ...
  document.title = `${klassName} ${subjectName} â ${chapterTitle} Theory Test`;
  $("#chapterMetaBadge").innerHTML = `<i class="fa-solid fa-graduation-cap"></i> ${chapterBadge} â¢ Theory`;
...
async function submitTest(auto) {
  ...
  $("#resStatus").textContent = result === "Pass" ? "ð Passed (Self-Assessed)" : "Keep Practicing";
  ...
  state.questions.forEach((q, i) => {
    ...
    div.innerHTML = `
      ...
      <div class="pill ${isCorrect ? "adm-pill-pass" : "adm-pill-fail"}" style="display:inline-block;padding:3px 10px;border-radius:99px;font-size:11.5px;font-weight:700">
        ${isCorrect ? "â Self-Assessed Correct" : "â Self-Assessed Incorrect"}
      </div>`;
...
$("#shareBtn").addEventListener("click", async () => {
  const text = `I got ${state.pct}% in my ${klassName} ${subjectName} â ${chapterTitle} Theory Self-Assessment Test! ð`;
...
$("#certBtn").addEventListener("click", () => {
  ...
  $("#certBodyText").innerHTML = `for outstanding performance in the <b>${escapeHtml(klassName)} ${escapeHtml(subjectName)} â ${escapeHtml(chapterTitle)} (Theory)</b> practice test, securing <b id="cPct">${state.pct}%</b> with <b id="cCorrect">${$("#sCorrect").textContent}</b> correct answers out of ${state.questions.length}.`;
  show("screen-cert");
});
...
async function admAction(act, id) {
  ...
  try {
    if (act === "view") {
      await modal({
        title: "Theory Scorecard â " + (row.name || ""),
    ...
    if (act === "cert" || act === "certdl") {
      ...
      $("#certBodyText").innerHTML = `for outstanding performance in the <b>${escapeHtml(klassName)} ${escapeHtml(subjectName)} â ${escapeHtml(chapterTitle)} (Theory)</b> practice test, securing <b id="cPct">${row.percentage}%</b> with <b id="cCorrect">${row.correct}</b> correct answers out of ${state.questions.length || TOTAL_Q}.`;
...
$("#expPdf").addEventListener("click", () => {
  try {
    ...
    pdf.setFontSize(14);
    pdf.text(`${klassName} ${subjectName} ${chapterTitle} â Theory Admin Report`, 40, 40);
    pdf.setFontSize(9);
