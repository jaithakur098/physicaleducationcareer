...
(function (root) {
  ...
  // Persist sessions across reloads (browser localStorage). Some browsers block
  // this in private mode — we swallow the error so login itself still works.
  try { auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL); } catch(e){}

  function serverTs(){ return firebase.firestore.FieldValue.serverTimestamp(); }

  // Map opaque Firebase auth error codes to human-readable messages so the
  // student panel never surfaces something like "Firebase: auth/quota-exceeded".
