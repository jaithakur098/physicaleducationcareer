...
(function (root) {
  ...
  function variants(v){
    var QB=getQB(); if (QB && QB.variants) return QB.variants(v);
    var s = norm(v);
    if (!s) return [];
    var out = [s];
  ...
  function questionMatches(q, classId, subjectId, chapterId){
    var QB=getQB(); if (QB && QB.questionMatches) return QB.questionMatches(q, classId, subjectId, chapterId);
    if (classId){
  ...
  async function fetchAllQuestions(){
    var QB=getQB(); if (QB && QB.fetchAllQuestions) return QB.fetchAllQuestions();
    var snap = await db.collection('questions').get();
    return snap.docs.map(function(d){ return Object.assign({id:d.id}, d.data()); });
  }

  function filterWithDebug(all, classId, subjectId, chapterId, label){
    var QB=getQB(); if (QB && QB.filterQuestions) return QB.filterQuestions(all, classId, subjectId, chapterId, label);
    var matched = [];
    var rejected = [];
  ...
  var Practice = {
    ...
    listAvailableQuestions: async function (classId, subjectId, chapterId) {
      var QB=getQB(); if (QB && QB.listAvailableQuestions) return QB.listAvailableQuestions(classId, subjectId, chapterId);
      var all = await fetchAllQuestions();
      return filterWithDebug(all, classId, subjectId, chapterId, 'listAvailableQuestions');
    },
    getRandomQuestions: async function (test) {
      var QB=getQB(); if (QB && QB.getQuestionsForTest) {
    ...
    submitAttempt: async function (payload) {
      // Practice attempts also enter the admin approval workflow so leaderboard,
      // rank, certificates and statistics stay in sync with Live tests.
      var doc = Object.assign({ status: 'pending', type: 'practice' }, payload, { submittedAt: serverTs() });
      var ref = await db.collection('practice_attempts').add(doc);
      return ref.id;
    },
    ...
    // Leaderboard: APPROVED attempts only, one row per student (best score).
