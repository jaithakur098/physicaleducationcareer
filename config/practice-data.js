...
(function (root) {
  ...
  function variants(v){
    if (QB && QB.variants) return QB.variants(v);
    var s = norm(v);
    if (!s) return [];
    var out = [s];
  ...
  function questionMatches(q, classId, subjectId, chapterId){
    if (QB && QB.questionMatches) return QB.questionMatches(q, classId, subjectId, chapterId);
    if (classId){
  ...
  async function fetchAllQuestions(){
    if (QB && QB.fetchAllQuestions) return QB.fetchAllQuestions();
    var snap = await db.collection('questions').get();
    return snap.docs.map(function(d){ return Object.assign({id:d.id}, d.data()); });
  }

  function filterWithDebug(all, classId, subjectId, chapterId, label){
    if (QB && QB.filterQuestions) return QB.filterQuestions(all, classId, subjectId, chapterId, label);
    var matched = [];
    var rejected = [];
  ...
  var Practice = {
    ...
    listAvailableQuestions: async function (classId, subjectId, chapterId) {
      if (QB && QB.listAvailableQuestions) return QB.listAvailableQuestions(classId, subjectId, chapterId);
      var all = await fetchAllQuestions();
