...
(function (root) {
  ...
  function normalizeQuestion(raw) {
    ...
    // Guard: if a class/subject/chapter value looks like question text (very long
    // or contains a question mark) AND question is empty, swap them.
    function looksLikeQuestion(v) {
      return v && (v.length > 60 || /[?]/.test(v) || v.split(/\s+/).length > 8);
    }
    ...
    // If class/subject/chapter still hold obvious question text, drop them so
    // the row doesn't poison Firestore. Test Builder filters will then ignore
    // this row until an admin edits it — safer than a silent bad match.
