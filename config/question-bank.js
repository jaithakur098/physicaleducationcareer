...
(function (root) {
  ...
  function optionValue(q, key, idx) {
    // Accept every shape a bulk-imported / manually-saved document might use:
    //   a / A            (legacy short keys)
    //   optionA / OptionA / option_a  (modern long keys, CSV/Excel headers)
    //   options[idx]     (array shape)
    var lower = key;                   // 'a' 'b' 'c' 'd'
    var upper = key.toUpperCase();     // 'A' 'B' 'C' 'D'
    var long1 = 'option' + upper;      // 'optionA'
    var long2 = 'Option' + upper;      // 'OptionA'
    var long3 = 'option_' + lower;     // 'option_a'
    var keys = [lower, upper, long1, long2, long3];
    for (var i = 0; i < keys.length; i++) {
      var v = q[keys[i]];
      if (v !== undefined && v !== null && v !== '') return v;
    }
    if (Array.isArray(q.options) && q.options[idx] !== undefined && q.options[idx] !== null && q.options[idx] !== '') {
  ...
  function correctIndex(q) {
    // Look at every alias that could hold the correct answer.
    var raw;
    if (q.answer !== undefined && q.answer !== null && q.answer !== '') raw = q.answer;
    else if (q.correct !== undefined && q.correct !== null && q.correct !== '') raw = q.correct;
    else if (q.correctAnswer !== undefined && q.correctAnswer !== null && q.correctAnswer !== '') raw = q.correctAnswer;
    else if (q.correctOption !== undefined && q.correctOption !== null && q.correctOption !== '') raw = q.correctOption;
    else raw = q.Answer !== undefined ? q.Answer : q.Correct;
