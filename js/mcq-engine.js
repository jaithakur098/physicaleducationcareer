...
import {
  ...
  orderBy, limit, serverTimestamp, doc, getDoc, updateDoc, deleteDoc
...
async function initEngine() {
  ...
  const testId = params.get("testId") || params.get("liveId");
  if (testId && await initFirestoreLiveTest(testId)) return;
...
async function initFirestoreLiveTest(testId) {
  if (!db) return false;
  try {
    const testSnap = await getDoc(doc(db, "live_tests", testId));
    if (!testSnap.exists()) return false;
    const test = { id: testSnap.id, ...testSnap.data() };
    if (test.isPublished === false) {
      await modal({ title: "Test Not Available", msg: "This live test is not published yet." });
      return true;
    }

    let liveQuestions = [];
    if (window.QuestionBank && QuestionBank.getQuestionsForTest) {
      liveQuestions = await QuestionBank.getQuestionsForTest(test);
      liveQuestions = liveQuestions.map(q => QuestionBank.toEngineQuestion ? QuestionBank.toEngineQuestion(q) : q);
    } else {
      const snap = await getDocs(collection(db, "questions"));
      const all = [];
      snap.forEach(d => all.push({ id: d.id, ...d.data() }));
      const byId = {};
      all.forEach(q => { if (q && q.id) byId[q.id] = q; });
      liveQuestions = (test.questionIds && test.questionIds.length)
        ? test.questionIds.map(id => byId[id]).filter(Boolean)
        : all.filter(q => (!test.classId || (q.classId || q.cls) === test.classId) &&
                          (!test.subjectId || (q.subjectId || q.subject) === test.subjectId) &&
                          (!test.chapterId || (q.chapterId || q.chapter) === test.chapterId));
    }

    window.questionBank = liveQuestions;
    TOTAL_Q = liveQuestions.length || Number(test.totalQuestions) || 0;
    TIME_LIMIT = (Number(test.durationMin) || 45) * 60;
    COLLECTION = "student_attempts";
    chapterTitle = test.title || "Live Test";
    chapterBadge = test.title || "Live Test";
    klassName = test.classId || test.className || "Class";
    subjectName = test.subjectId || test.subject || "Physical Education";

    document.title = `${chapterTitle} — Live Test`;
    $("#chapterMetaBadge").innerHTML = `<i class="fa-solid fa-broadcast-tower"></i> ${escapeHtml(chapterBadge)}`;
    $("#chapterTestTopBadge").innerHTML = `<i class="fa-solid fa-book"></i> ${escapeHtml(chapterBadge)}`;
    $("#chapterHeading").innerHTML = escapeHtml(chapterTitle);
    $(".hero-stats").innerHTML = `<div class="stat"><b>${TOTAL_Q}</b><span>Questions</span></div><div class="stat"><b>${Math.round(TIME_LIMIT / 60)}</b><span>Minutes</span></div><div class="stat"><b>90%+</b><span>Certificate</span></div>`;

    const classSel = $("#fClass");
    if (classSel) classSel.value = test.className || test.classId || classSel.value;
    loadLeaderboard();
    return true;
  } catch (err) {
    console.error("Failed to load live test:", err);
    await modal({ title: "Loading Error", msg: "Failed to load this live test. Please refresh and try again." });
    return true;
  }
}
...
function buildQuestions() {
  ...
  const picked = shuffle(window.questionBank).slice(0, TOTAL_Q || window.questionBank.length);
  state.questions = picked.map(q => {
    const opts = Array.isArray(q.options) ? q.options : [q.a, q.b, q.c, q.d];
    const answer = Number.isFinite(q.answer) ? q.answer : 0;
    const optsWithIdx = opts.map((o, i) => ({ o, i }));
    const shuffled = shuffle(optsWithIdx);
    return {
      question: q.question,
      options: shuffled.map(x => x.o),
      correct: shuffled.findIndex(x => x.i === answer)
    };
  });
  TOTAL_Q = state.questions.length;
